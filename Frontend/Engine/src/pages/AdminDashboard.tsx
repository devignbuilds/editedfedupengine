...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        "http://localhost:5001/api/dashboard/admin/stats",
        {
  ...
  const normalizedData = trendData.map(
    (val) => (val - minVal) / (maxVal - minVal),
  );
  const points = normalizedData
    .map(
      (val, index) =>
        `${index * (100 / (trendData.length - 1))},${100 - val * 100}`,
    )
    .join(" ");
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span
                      className={`inline-flex items-center text-xs font-medium ${
                        metric.trend === "up"
                          ? "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-1.5 py-0.5 rounded-md"
                          : metric.trend === "down"
                            ? "text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-1.5 py-0.5 rounded-md"
              ...
              <div className="h-[300px] w-full mt-4 flex items-end justify-center px-4 relative">
                ...
                <div className="absolute inset-x-4 top-0 bottom-0 flex flex-col justify-between text-muted-foreground/20 pointer-events-none">
                  {[0, 1, 2, 3, 4].map((_, i) => (
                    <div
                      key={i}
                      className="border-t border-dashed w-full h-px"
                    ></div>
                  ))}
                </div>
                ...
                {stats.recentProjects && stats.recentProjects.length > 0
                  ? (stats as any).recentProjects.map((p: any) => (
                      <Link
                        key={p._id}
                        to={`/projects/${p._id}`}
                        className="flex items-start space-x-4 p-2 rounded-xl hover:bg-muted/50 transition-colors"
                      >
                        ...
                        <div className="space-y-1 flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {p.client?.name || "Unknown Client"} • Recently
                            updated
                          </p>
