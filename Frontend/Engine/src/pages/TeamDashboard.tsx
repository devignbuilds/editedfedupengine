      ...
      try {
        const [projectsRes, usersRes, tasksRes] = await Promise.all([
          ...
          fetch("http://localhost:5000/api/tasks", {
            // Assuming this returns all accessible tasks
            headers: { Authorization: `Bearer ${user?.token}` },
          }),
        ]);
    ...
    <DashboardLayout>
      <div className="mb-12">
        ...
        <p className="text-muted-foreground mt-1">
          Manage your team members and collaborative projects.
        </p>
      </div>
      ...
      <div className="mb-12">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <Link
            to="/tasks"
            className="ml-auto text-xs font-bold uppercase tracking-widest text-primary hover:underline"
          >
            View Board
          </Link>
        </div>
        ...
        {myTasks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {myTasks.slice(0, 4).map((task) => (
              <div
                key={task._id}
                className="p-4 rounded-xl border border-border bg-card hover:border-foreground/50 transition-colors"
              >
                ...
                <p className="text-xs text-muted-foreground truncate opacity-70 mb-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center space-x-2 text-muted-foreground">
                  ...
                  <span className="text-xs font-bold">
                    {
                      projects.filter((p) =>
                        p.employees?.some((e: any) => e._id === member._id),
                      ).length
                    }{" "}
                    Active Projects
                  </span>
            ...
            >
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="font-black text-xl tracking-tight group-hover:underline underline-offset-4">
                    {project.name}
                  </h3>
                  <span
