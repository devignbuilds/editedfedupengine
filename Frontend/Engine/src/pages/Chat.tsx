...
export default function Chat() {
  ...
  return (
    <DashboardLayout>
      <div className="h-[calc(100vh-12rem)] flex overflow-hidden border border-border rounded-[2rem] bg-card shadow-2xl">
        {/* Chat Sidebar */}
        <aside className="w-80 border-r border-border bg-muted/20 flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            {projects.map((p) => (
              ...
              >
                <Hash className={`h-4 w-4 shrink-0 ${selectedProject?._id === p._id ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-primary'} transition-colors`} />
                <span className="font-bold text-sm truncate">{p.name}</span>
              </button>
            ))}
            {!loading && projects.length === 0 && (
              <div className="p-4 text-center">
                <p className="text-xs text-muted-foreground italic">No active channels.</p>
              </div>
            )}
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col bg-background/30 max-h-full">
           {selectedProject ? (
             <>
               <header className="h-20 border-b border-border px-8 flex items-center justify-between bg-background/40 backdrop-blur-md sticky top-0 z-10">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        <h3 className="font-bold text-lg leading-none">{selectedProject.name}</h3>
               ...
               <div className="flex-1 overflow-hidden p-6">
                <ChatWindow key={selectedProject._id} projectId={selectedProject._id} />
               </div>
             </>
           ) : (
             <div className="flex-1 flex flex-col items-center justify-center text-center p-12 space-y-6">
                ...
                <div>
                   <h3 className="text-2xl font-black tracking-tight mb-2">Initialize Communication</h3>
                   <p className="text-muted-foreground max-w-xs mx-auto leading-relaxed">
                      Select a project channel to start real-time synchronization with your team and clients.
                   </p>
                </div>
             </div>
           )}
